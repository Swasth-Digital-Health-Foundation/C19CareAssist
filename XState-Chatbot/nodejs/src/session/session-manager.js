...
class SessionManager {
  async fromUser(reformattedMessage) {
    ...
    let event = false || intention == 'reset' ? 'USER_RESET' : 'USER_MESSAGE';
  ...
  async toUser(user, outputMessages, extraInfo) {
    ...
    for (let message of outputMessages) {
      telemetry.log(user.userId, 'to_user', {
        message: { type: 'text', output: message },
      });
    }
  }
...
let grammer = {
  reset: [{ intention: 'reset', recognize: ['reset', 'hi', 'hello'] }],
};
