...
const generateToken = (param: any, data: any) => {
  const token = jwt.sign(
    { exp: moment.utc().add(param.ttl, 'minute').valueOf(), data: data },
    param.secret,
    { algorithm: param.algorithm, issuer: 'healthgateway' }
  );
  return token;
};
...
class AuthHelper {
  ...
  generateApiToken(data: object) {
    const token = generateToken(
      {
        ttl: 10,
        secret: Buffer.from(API_TOKEN_PRIVATEKEY, 'base64').toString('ascii'),
        algorithm: 'RS256',
      },
      data
    );
  ...
  generateAccessToken(data: object) {
    const token = generateToken(
      { ttl: 10, secret: ACCESSTOKEN_SECRET, algorithm: 'HS256' },
      data
    );
    return token;
  }
  ...
  generateGatewayToken(secret: string, data: object) {
    const token = generateToken(
      { ttl: 10, secret: secret, algorithm: 'HS256' },
      data
    );
    return token;
  }
  ...
  generateRSAKeyPair() {
    ...
    const publicKey = Buffer.from(key.exportKey('pkcs8-public-pem')).toString(
      'base64'
    );
    const privateKey = Buffer.from(key.exportKey('pkcs8-pem')).toString(
      'base64'
    );
