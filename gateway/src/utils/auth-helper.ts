...
const generateToken = (param: any, data: any) => {
    const token = jwt.sign({ exp: moment.utc().add(param.ttl, "minute").unix(), data: data }, param.secret, { algorithm: param.algorithm, issuer: "healthgateway" });
    return token;
};
...
class AuthHelper {
    verify(type: string, token: string) {
        ...
        try {
            const data = jwt.verify(token, secret, { maxAge, algorithms: ["HS256"] });
            const hasExpired = moment.unix(data.exp).isBefore(moment.utc());
    ...
    generateApiToken(data: object) {
        const token = generateToken({ ttl: 30, secret: Buffer.from(API_TOKEN_PRIVATEKEY, "base64").toString("ascii"), algorithm: "RS256" }, data);
        return token;
    }

    generateAccessToken(data: object) {
        const token = generateToken({ ttl: 30, secret: ACCESSTOKEN_SECRET, algorithm: "HS256" }, data);
        return token;
    }
    ...
    generateGatewayToken(secret: string, data: object) {
        const token = generateToken({ ttl: 30, secret: secret, algorithm: "HS256" }, data);
        return token;
    }
